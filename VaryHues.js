...
if (len == 0) {
 ...
 if (app.activeDocument.documentColorSpace != "DocumentColorSpace.RGB"){
	alert('This script will only work on documents in the RGB colorspace.');
 }else{
	...
	var data = prompt(line1+line2+line3, "100");
        ...
        for (j=0; j<len; j++) {
            ...
            if (isNumber(action[0]) && isNumber(action[1]) && action.length == 2) {
                ...
                if(app.activeDocument.documentColorSpace == "DocumentColorSpace.RGB"){
                    ...
                    if(ob[j].typename === 'GroupItem'){
                        processGroup(ob[j]);
                    }else
                    if(ob[j].typename === 'CompoundPathItem'){
                        processCompoundPath(ob[j]);
                    }else{
                        processPathItem(ob[j]);
                    }
                }
            } else {
                //break
...
function processPathItem(ob){
         if (strokes == false) {
             if (ob.filled == true) {
                 setColor(ob.fillColor, setLegal(action[0], 255));
             }
         } else {
             if (ob.stroked == true) {
                 setColor(ob.strokeColor, setLegal(action[0],255));
             }
         }
}
function processGroup(ob){
    for(var p=0;p < ob.pageItems.length;p++){
        if(ob.pageItems[p].typename == 'PathItem'){
            processPathItem(ob.pageItems[p]);
        } else  if(ob.pageItems[p].typename == 'CompoundPathItem'){
            processCompoundPath(ob.pageItems[p]);
        } else if(ob.pageItems[p].typename == 'GroupItem'){
            processGroup(ob.pageItems[p]);
        }
    }
}

function processCompoundPath(ob){
    for(var c=0;c<ob.pathItems.length;c++){
       // alert(ob.pathItems[c].typename);
        if(ob.pathItems[c].typename == 'PathItem'){
            processPathItem(ob.pathItems[c]);
        } 
    }
}
function error(val) {
